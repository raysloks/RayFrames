...
function RayCastingBarMixin:OnLoad(unit, showTradeSkills, showShield)
	self.StagePoints = {};
	self.StagePips = {};
	self.StageTiers = {};
...
function RayCastingBarMixin:AddWidgetForFade(widget)
...
function RayCastingBarMixin:SetUnit(unit, showTradeSkills, showShield)
...
function RayCastingBarMixin:OnShow()
	if ( self.unit ) then
...
function RayCastingBarAnim_OnInterruptSparkAnimFinish(self)
	local castingBar = self:GetParent();
	castingBar:SetValue(castingBar.maxValue);
	castingBar:HideSpark();
end

function RayCastingBarAnim_OnFadeOutFinish(self)
	local castingBar = self:GetParent();
	castingBar:Hide();
end

function RayCastingBarMixin:GetEffectiveType(isChannel, notInterruptible, isTradeSkill, isEmpowered)
...
function RayCastingBarMixin:GetTypeInfo(barType)
	if not barType then
		barType = "standard";
	end
	return RAY_CASTING_BAR_TYPES[barType];
end

function RayCastingBarMixin:OnEvent(event, ...)
	local arg1 = ...;
...
function RayCastingBarMixin:UpdateInterruptibleState(notInterruptible)
...
function RayCastingBarMixin:OnUpdate(elapsed)
...
function RayCastingBarMixin:ApplyAlpha(alpha)
	self:SetAlpha(alpha);
...
function RayCastingBarMixin:FinishSpell()
...
function RayCastingBarMixin:ShowSpark()
	if ( self.Spark ) then
		self.Spark:Show();
	end
	...
	for barType, barTypeInfo in pairs(RAY_CASTING_BAR_TYPES) do
...
function RayCastingBarMixin:HideSpark()
	if ( self.Spark ) then
		self.Spark:Hide();
	end
...
function RayCastingBarMixin:PlayInterruptAnims()
...
function RayCastingBarMixin:StopInterruptAnims()
	if self.HoldFadeOutAnim then
...
function RayCastingBarMixin:PlayFadeAnim()
...
function RayCastingBarMixin:PlayFinishAnim()
	if not self.playCastFX then
		return;
	end
...
function RayCastingBarMixin:StopFinishAnims()
	if self.FlashAnim then
		self.FlashAnim:Stop();
	end
	...
	for _, barTypeInfo in pairs(RAY_CASTING_BAR_TYPES) do
		local finishAnim = barTypeInfo.finishAnim and self[barTypeInfo.finishAnim];
		if finishAnim then
			finishAnim:Stop();
		end
...
function RayCastingBarMixin:StopAnims()
	self:StopInterruptAnims();
	self:StopFinishAnims();
end

function RayCastingBarMixin:UpdateIsShown()
...
function RayCastingBarMixin:SetAndUpdateShowCastbar(showCastbar)
	self.showCastbar = showCastbar;
	self:UpdateIsShown();
end

function RayCastingBarMixin:SetLook(look)
...
function RayCastingBarMixin:AddStages(numStages)
	...
	for i = 1,self.NumStages-1,1 do
		...
		if(duration > CASTBAR_STAGE_DURATION_INVALID) then
			...
			if not stagePip then
				stagePip = CreateFrame("FRAME", nil, self, hasFX and "RayCastingBarFrameStagePipFXTemplate" or "RayCastingBarFrameStagePipTemplate");
	...
	for i = 1,self.NumStages-1,1 do
		...
		if not chargeTier then
			chargeTier = CreateFrame("FRAME", nil, self, "RayCastingBarFrameStageTierTemplate");
...
function RayCastingBarMixin:UpdateStage()
	local maxStage = 0;
...
function RayCastingBarMixin:ClearStages()
...
PlayerRayCastingBarMixin = {};

function PlayerRayCastingBarMixin:OnLoad()
	local showTradeSkills = true;
	local showShieldNo = false;
	RayCastingBarMixin.OnLoad(self, "player", showTradeSkills, showShieldNo);
	self.Icon:Hide();
end

function PlayerRayCastingBarMixin:OnShow()
	RayCastingBarMixin.OnShow(self);
...
OverlayPlayerRayCastingBarMixin = {};

function OverlayPlayerRayCastingBarMixin:OnLoad()
	local showTradeSkills = true;
	local showShieldNo = false;
	RayCastingBarMixin.OnLoad(self, "player", showTradeSkills, showShieldNo);
	self.Icon:Hide();
...
--[[
--	Call to use this casting bar over the specified frame INSTEAD of showing the default PlayerRayCastingBar.
--	Will display any currently active Player cast, and any future Player casts until EndReplacingPlayerBar is called.
--]]
function OverlayPlayerRayCastingBarMixin:StartReplacingPlayerBarAt(parentFrame, overrideBarType, overrideAnchor)
	-- Disable real Player Cast Bar
	PlayerRayCastingBarFrame:SetAndUpdateShowCastbar(false);
...
--[[
--	Call to resume using only the default PlayerRayCastingBar.
--	PlayerRayCastingBar will immediately pick up displaying any already-active Player casts.
--]]
function OverlayPlayerRayCastingBarMixin:EndReplacingPlayerBar()
	-- Hide self
	...
	PlayerRayCastingBarFrame:SetAndUpdateShowCastbar(true);
end

-- Override template mixin for overriden bar type
function OverlayPlayerRayCastingBarMixin:GetEffectiveType(isChannel, notInterruptible, isTradeSkill, isEmpowered)
	return self.overrideBarType or RayCastingBarMixin.GetEffectiveType(self, isChannel, notInterruptible, isTradeSkill, isEmpowered);
end

function OverlayPlayerRayCastingBarMixin:OnShow()
	RayCastingBarMixin.OnShow(self);
	EventRegistry:TriggerEvent("OverlayPlayerCastBar.OnShow");
end

function OverlayPlayerRayCastingBarMixin:OnHide()
	RayCastingBarMixin.OnHide(self);
