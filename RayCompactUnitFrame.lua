...
RAY_BOSS_DEBUFF_SIZE_INCREASE = 9;
RAY_CUF_READY_CHECK_DECAY_TIME = 11;
RAY_DISTANCE_THRESHOLD_SQUARED = 250*250;
RAY_CUF_NAME_SECTION_SIZE = 15;
RAY_CUF_AURA_BOTTOM_OFFSET = 1;
RAY_CUF_SELECTION_TYPE_COLORS = {
	{0.90, 0.15, 0.15},
	{0.95, 0.75, 0.25},
	{0.85, 0.75, 0.05},
	{0.25, 0.85, 0.25},
...
function RayCompactUnitFrame_OnLoad(self)
	...
	RayCompactUnitFrame_SetOptionTable(self, OPTION_TABLE_NONE);

	if not self.disableMouse then
		RayCompactUnitFrame_SetUpClicks(self);

		-- for i, buffFrame in pairs(self.buffFrames) do
		-- 	buffFrame.dropDown = self.dropDown;
		-- 	CompactUnitFrame_SetUpClicks(buffFrame);
		-- end
		-- for i, debuffFrame in pairs(self.debuffFrames) do
		-- 	debuffFrame.dropDown = self.dropDown;
		-- 	CompactUnitFrame_SetUpClicks(debuffFrame);
		-- end
		-- for i, dispelDebuffFrame in pairs(self.dispelDebuffFrames) do
		-- 	RayCompactUnitFrame_SetUpClicks(dispelDebuffFrame, self);
		-- end
	end
end

function RayCompactUnitFrame_OnEvent(self, event, ...)
	local arg1, arg2, arg3, arg4 = ...;
	if ( event == self.updateAllEvent and (not self.updateAllFilter or self.updateAllFilter(self, event, ...)) ) then
		RayCompactUnitFrame_UpdateAll(self);
	elseif ( event == "PLAYER_ENTERING_WORLD" ) then
		RayCompactUnitFrame_UpdateAll(self);
	elseif ( event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_SOFT_ENEMY_CHANGED" or event == "PLAYER_SOFT_FRIEND_CHANGED" ) then
		if ( self.targetSources["player"] ) then
			RayCompactUnitFrame_UpdateAll(self);
		else
			RayCompactUnitFrame_UpdateSelectionHighlight(self);
			RayCompactUnitFrame_UpdateName(self);
			RayCompactUnitFrame_UpdateWidgetsOnlyMode(self);
			RayCompactUnitFrame_UpdateHealthBorder(self);
			RayCompactUnitFrame_UpdateWidgetSet(self);
		end
	elseif ( event == "PLAYER_REGEN_ENABLED" or event == "PLAYER_REGEN_DISABLED" ) then
		RayCompactUnitFrame_UpdateAuras(self);	--We filter differently based on whether the player is in Combat, so we need to update when that changes.
	elseif ( event == "PLAYER_ROLES_ASSIGNED" ) then
		RayCompactUnitFrame_UpdateRoleIcon(self);
		RayCompactUnitFrame_UpdatePowerBarVisible(self); -- I'll probably want a more flexible system for things like this in the future
	elseif ( event == "READY_CHECK" ) then
		RayCompactUnitFrame_UpdateReadyCheck(self);
	elseif ( event == "READY_CHECK_FINISHED" ) then
		RayCompactUnitFrame_FinishReadyCheck(self);
	elseif ( event == "PARTY_MEMBER_DISABLE" or event == "PARTY_MEMBER_ENABLE" ) then	--Alternate power info may now be available.
		RayCompactUnitFrame_UpdateMaxPower(self);
		RayCompactUnitFrame_UpdatePower(self);
		RayCompactUnitFrame_UpdatePowerColor(self);
	elseif ( event == "UNIT_TARGET" and arg1 ~= nil and not UnitIsUnit(arg1, "player") and self.targetSources[arg1] ) then
		RayCompactUnitFrame_UpdateAll(self);
	elseif ( event == "UNIT_SPELLCAST_SENT" ) then
		if ( UnitIsUnit(arg1, "player") and arg2 ~= nil and UnitIsUnit(arg2, self.unit) ) then
			self.castTargetHighlight:Show();
			self.castTargetCastGUID = arg3;
		end
	elseif ( event == "UNIT_SPELLCAST_FAILED" or event == "UNIT_SPELLCAST_INTERRUPTED" or event == "UNIT_SPELLCAST_STOP" or event == "UNIT_SPELLCAST_SUCCEEDED" ) then
		if ( self.castTargetCastGUID == arg2 ) then
			self.castTargetHighlight:Hide();
		end
	else
		local unitMatches = arg1 == self.unit or arg1 == self.displayedUnit;
		if ( unitMatches ) then
			if ( event == "UNIT_MAXHEALTH" ) then
				RayCompactUnitFrame_UpdateMaxHealth(self);
				RayCompactUnitFrame_UpdateHealth(self);
				RayCompactUnitFrame_UpdateHealPrediction(self);
			elseif ( event == "UNIT_HEALTH" ) then
				RayCompactUnitFrame_UpdateHealth(self);
				RayCompactUnitFrame_UpdateStatusText(self);
				RayCompactUnitFrame_UpdateHealPrediction(self);
			...
			elseif ( event == "UNIT_NAME_UPDATE" ) then
				RayCompactUnitFrame_UpdateHealth(self);		--This may signify that the unit is a new pet who replaced an old pet, and needs a health update
				RayCompactUnitFrame_UpdateHealthColor(self);	--This may signify that we now have the unit's class (the name cache entry has been received).
			elseif ( event == "UNIT_AURA" ) then
				local unitAuraUpdateInfo = arg2;
				RayCompactUnitFrame_UpdateAuras(self, unitAuraUpdateInfo);
			elseif ( event == "UNIT_THREAT_SITUATION_UPDATE" ) then
				RayCompactUnitFrame_UpdateAggroHighlight(self);
				RayCompactUnitFrame_UpdateAggroFlash(self);
				RayCompactUnitFrame_UpdateHealthBorder(self);
			elseif ( event == "UNIT_THREAT_LIST_UPDATE" ) then
				if ( self.optionTable.considerSelectionInCombatAsHostile ) then
					RayCompactUnitFrame_UpdateHealthColor(self);
					RayCompactUnitFrame_UpdateName(self);
				end
				RayCompactUnitFrame_UpdateAggroFlash(self);
				RayCompactUnitFrame_UpdateHealthBorder(self);
			...
			elseif ( event == "INCOMING_RESURRECT_CHANGED" ) then
				RayCompactUnitFrame_UpdateCenterStatusIcon(self);
			...
			elseif ( event == "UNIT_CLASSIFICATION_CHANGED" ) then
				RayCompactUnitFrame_UpdateClassificationIndicator(self);
...
function RayCompactUnitFrame_OnUpdate(self, elapsed)
	RayCompactUnitFrame_UpdateInRange(self);
	RayCompactUnitFrame_UpdateDistance(self);
	RayCompactUnitFrame_CheckReadyCheckDecay(self, elapsed);
	RayCompactUnitFrame_UpdateMouseover(self);

	if self.aurasDirty then
		RayCompactUnitFrame_UpdateAuras(self);
		self.aurasDirty = false;
	end
	
	local animatedLossBar = self.healthBar.AnimatedLossBar;
	if animatedLossBar then
		animatedLossBar:UpdateLossAnimation(self.healthBar:GetValue());
	end
end

--Externally accessed functions
function RayCompactUnitFrame_SetUnit(frame, unit)
	if ( unit ~= frame.unit or frame.hideCastbar ~= frame.optionTable.hideCastbar ) then
		...
		frame.targetSources = {};
		if ( unit == "target" ) then
			frame.targetSources["player"] = true;
		end

		for i, buffFrame in pairs(frame.buffFrames) do
			buffFrame.unit = unit;
			buffFrame:SetAttribute("unit", unit);
		end
		for i, debuffFrame in pairs(frame.debuffFrames) do
			debuffFrame.unit = unit;
			debuffFrame:SetAttribute("unit", unit);
		end
		-- for i, dispelDebuffFrame in pairs(frame.dispelDebuffFrames) do
		-- 	dispelDebuffFrame:SetAttribute("unit", unit);
		-- end
		
		frame.healthBarAnimatedLoss:SetUnitHealthBar(unit, frame.healthBar);

		if ( unit ) then
			RayCompactUnitFrame_RegisterEvents(frame);
		else
			RayCompactUnitFrame_UnregisterEvents(frame);
		end
		...
		RayCompactUnitFrame_UpdateAll(frame);
	end
end
...
function RayCompactUnitFrame_SetUpdateAllOnUpdate(self, doUpdate)
	if ( doUpdate ) then
		if ( not self.onUpdateFrame ) then
			...
			self.onUpdateFrame.func = function(updateFrame, elapsed) if ( self.displayedUnit ) then RayCompactUnitFrame_UpdateAll(self) end end;
		end
...
function RayCompactUnitFrame_SetUpFrame(frame, func, ...)
	func(frame, ...);
	RayCompactUnitFrame_UpdateAll(frame);
end

function RayCompactUnitFrame_SetOptionTable(frame, optionTable)
	frame.optionTable = optionTable;
	frame.aurasDirty = true;
end

function RayCompactUnitFrame_RegisterEvents(frame)
	local onEventHandler = frame.OnEvent or RayCompactUnitFrame_OnEvent;
	frame:SetScript("OnEvent", onEventHandler);

	RayCompactUnitFrame_UpdateUnitEvents(frame);

	local onUpdate = frame.OnUpdate or RayCompactUnitFrame_OnUpdate;
	frame:SetScript("OnUpdate", onUpdate);
end

function RayCompactUnitFrame_UpdateUnitEvents(frame)
	...
	frame:RegisterUnitEvent("UNIT_TARGETABLE_CHANGED", unit, displayedUnit);
	local targetSourcesArray = {};
	for k in pairs(frame.targetSources) do
		table.insert(targetSourcesArray, k);
	end
	frame:RegisterUnitEvent("UNIT_TARGET", unpack(targetSourcesArray));
end

function RayCompactUnitFrame_UnregisterEvents(frame)
	frame:SetScript("OnEvent", nil);
	frame:SetScript("OnUpdate", nil);
end

function RayCompactUnitFrame_SetUpClicks(frame)
	UIDropDownMenu_Initialize(frame.dropDown, RayCompactUnitFrameDropDown_Initialize, "MENU");
	RayCompactUnitFrame_SetMenuFunc(frame, frame);
	for i, buffFrame in pairs(frame.buffFrames) do
		RayCompactUnitFrame_SetMenuFunc(buffFrame, frame);
	end
	for i, debuffFrame in pairs(frame.debuffFrames) do
		RayCompactUnitFrame_SetMenuFunc(debuffFrame, frame);
	end
end

function RayCompactUnitFrame_SetMenuFunc(frame, mainFrame)
	frame:SetAttribute("*type1", "target");
    frame:SetAttribute("*type2", "menu");
	frame:RegisterForClicks("AnyDown");
	frame.menu = function()
		ToggleDropDownMenu(1, nil, mainFrame.dropDown, mainFrame:GetName(), 0, 0);
	end
end

function RayCompactUnitFrame_SetMaxBuffs(frame, numBuffs)
	frame.maxBuffs = numBuffs;
	frame.aurasDirty = true;
end

function RayCompactUnitFrame_SetMaxDebuffs(frame, numDebuffs)
	frame.maxDebuffs = numDebuffs;
	frame.aurasDirty = true;
end

function RayCompactUnitFrame_SetMaxDispelDebuffs(frame, numDispelDebuffs)
	frame.maxDispelDebuffs = numDispelDebuffs;
	frame.aurasDirty = true;
end

function RayCompactUnitFrame_SetUpdateAllEvent(frame, updateAllEvent, updateAllFilter)
...
--Update Functions
function RayCompactUnitFrame_UpdateAll(frame)
	RayCompactUnitFrame_UpdateInVehicle(frame);
	RayCompactUnitFrame_UpdateVisible(frame);
	if ( UnitExists(frame.displayedUnit) ) then
		RayCompactUnitFrame_UpdateMaxHealth(frame);
		RayCompactUnitFrame_UpdateHealth(frame);
		RayCompactUnitFrame_UpdateHealthColor(frame);
		RayCompactUnitFrame_UpdateMaxPower(frame);
		RayCompactUnitFrame_UpdatePower(frame);
		RayCompactUnitFrame_UpdatePowerColor(frame);
		RayCompactUnitFrame_UpdateWidgetsOnlyMode(frame);
		RayCompactUnitFrame_UpdateSelectionHighlight(frame);
		RayCompactUnitFrame_UpdateAggroHighlight(frame);
		RayCompactUnitFrame_UpdateAggroFlash(frame);
		RayCompactUnitFrame_UpdateHealthBorder(frame);
		RayCompactUnitFrame_UpdateInRange(frame);
		RayCompactUnitFrame_UpdateStatusText(frame);
		RayCompactUnitFrame_UpdateHealPrediction(frame);
		RayCompactUnitFrame_UpdateRoleIcon(frame);
		RayCompactUnitFrame_UpdateReadyCheck(frame);
		RayCompactUnitFrame_UpdateAuras(frame);
		RayCompactUnitFrame_UpdateCenterStatusIcon(frame);
		RayCompactUnitFrame_UpdateClassificationIndicator(frame);
		RayCompactUnitFrame_UpdateWidgetSet(frame);
		RayCompactUnitFrame_UpdatePowerBarVisible(frame);
	elseif (UnitIsGameObject(frame.displayedUnit) ) then -- Interactable GameObject
		RayCompactUnitFrame_HideHealth(frame);
		RayCompactUnitFrame_UpdateName(frame);
		RayCompactUnitFrame_UpdateWidgetsOnlyMode(frame);
		RayCompactUnitFrame_UpdateInRange(frame);
		RayCompactUnitFrame_UpdateStatusText(frame);
		RayCompactUnitFrame_UpdateCenterStatusIcon(frame);
...
function RayCompactUnitFrame_UpdateInVehicle(frame)
...
function RayCompactUnitFrame_UpdateVisible(frame)
	if ( UnitExists(frame.unit) or UnitExists(frame.displayedUnit) ) then
		...
		if ( not frame.optionTable.showAlways ) then
			frame:Show();
		else
		...
		RayCompactUnitFrame_ClearWidgetSet(frame);
		if ( not frame.optionTable.showAlways ) then
			frame:Hide();
		else
...
function RayCompactUnitFrame_IsTapDenied(frame)
...
function RayCompactUnitFrame_IsOnThreatListWithPlayer(unit)
	local _, threatStatus = UnitDetailedThreatSituation("player", unit);
	return IsOnThreatList(threatStatus);
end

function RayCompactUnitFrame_UpdateHealthColor(frame)
	local r, g, b;
	...
	elseif (UnitIsDead(frame.unit)) then
		...
		if ( frame.optionTable.healthBarColorOverride ) then
			...
			elseif ( RayCompactUnitFrame_IsTapDenied(frame) ) then
				-- Use grey if not a player and can't get tap on unit
				r, g, b = 0.9, 0.9, 0.9;
			elseif ( frame.optionTable.colorHealthBySelection ) then
				-- Use color based on the type of unit (neutral, etc.)
				if ( frame.optionTable.considerSelectionInCombatAsHostile and RayCompactUnitFrame_IsOnThreatListWithPlayer(frame.displayedUnit) ) then
					r, g, b = 1.0, 0.0, 0.0;
				elseif ( UnitIsPlayer(frame.displayedUnit) and UnitIsFriend("player", frame.displayedUnit) ) then
					...
					r, g, b = unpack(RAY_CUF_SELECTION_TYPE_COLORS[UnitSelectionType(frame.unit) + 1]);
...
function RayCompactUnitFrame_HideHealth(frame)
	frame:Show();
	frame.hideHealthbar = true;
	frame.healthBar:SetShown(false);
end

function RayCompactUnitFrame_UpdateMaxHealth(frame)
	...
	if ( frame.optionTable.smoothHealthUpdates ) then
		...
		if frame.healthBar.AnimatedLossBar then
			frame.healthBar.AnimatedLossBar:UpdateHealthMinMax();
		end
	end
...
function RayCompactUnitFrame_UpdateHealth(frame)
	...
	if ( frame.optionTable.smoothHealthUpdates ) then
		...
		local animatedLossBar = frame.healthBar.AnimatedLossBar;
		local previousValue = frame.healthBar:GetValue();
		PixelUtil.SetStatusBarValue(frame.healthBar, health);
		if ( animatedLossBar ) then
			animatedLossBar:UpdateHealth(frame.healthBar:GetValue(), previousValue);
		end
	end
	RayCompactUnitFrame_UpdateName(frame);
	RayCompactUnitFrame_UpdateHealthColor(frame);
...
local function RayCompactUnitFrame_GetDisplayedPowerID(frame)
...
function RayCompactUnitFrame_UpdateMaxPower(frame)
	if frame.powerBar then
		frame.powerBar:SetMinMaxValues(0, UnitPowerMax(frame.displayedUnit, RayCompactUnitFrame_GetDisplayedPowerID(frame)));
	end
end

function RayCompactUnitFrame_UpdatePower(frame)
	if frame.powerBar then
		PixelUtil.SetStatusBarValue(frame.powerBar, UnitPower(frame.displayedUnit, RayCompactUnitFrame_GetDisplayedPowerID(frame)));
	end
end

function RayCompactUnitFrame_UpdatePowerColor(frame)
...
function RayCompactUnitFrame_SetPowerBarVisible(frame, visible)
	if not frame.powerBar then
		return;
	end

	if ( visible ) then
		frame.healthBar:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -1, 1 + 8);
	else
		frame.healthBar:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -1, 1);
	end
end

function RayCompactUnitFrame_UpdatePowerBarVisible(frame)
	if not frame.powerBar then
		return;
	end

	if ( type(frame.optionTable.displayPowerBar) == "function" ) then
		RayCompactUnitFrame_SetPowerBarVisible(frame, frame.optionTable.displayPowerBar(frame));
	else
		RayCompactUnitFrame_SetPowerBarVisible(frame, frame.optionTable.displayPowerBar);
	end
end

function RayShouldShowName(frame)
...
function RayCompactUnitFrame_UpdateWidgetsOnlyMode(frame)
...
function RayCompactUnitFrame_UpdateName(frame)
	if frame.UpdateNameOverride and frame:UpdateNameOverride() then
		return;
	end

	if ( not RayShouldShowName(frame) ) then
		frame.name:Hide();
	else
		...
		if ( RayCompactUnitFrame_IsTapDenied(frame) or UnitIsDead(frame.unit) ) then
			-- Use grey if not a player and can't get tap on unit
...
function RayCompactUnitFrame_UpdateSelectionHighlight(frame)
...
function RayCompactUnitFrame_UpdateAggroHighlight(frame)
...
function RayCompactUnitFrame_UpdateHealthBorder(frame)
...
function RayCompactUnitFrame_UpdateAggroFlash(frame)
...
function RayCompactUnitFrame_UpdateInRange(frame)
...
function RayCompactUnitFrame_UpdateDistance(frame)
	local distance, checkedDistance = UnitDistanceSquared(frame.displayedUnit);

	if ( checkedDistance ) then
		local inDistance = distance < RAY_DISTANCE_THRESHOLD_SQUARED;
...
function RayCompactUnitFrame_UpdateStatusText(frame)
...
function RayCompactUnitFrame_UpdateHealPrediction(frame)
	...
	if ( myCurrentHealAbsorb > allIncomingHeal ) then
		...
		healAbsorbTexture = RayCompactUnitFrameUtil_UpdateFillBar(frame, healthTexture, frame.myHealAbsorb, shownHealAbsorb, -shownHealAbsorbPercent);
	...
	local incomingHealsTexture = RayCompactUnitFrameUtil_UpdateFillBar(frame, healthTexture, frame.myHealPrediction, myIncomingHeal, -myCurrentHealAbsorbPercent);
	--Append otherIncomingHeal on the health bar.
	incomingHealsTexture = RayCompactUnitFrameUtil_UpdateFillBar(frame, incomingHealsTexture, frame.otherHealPrediction, otherIncomingHeal);
	...
	RayCompactUnitFrameUtil_UpdateFillBar(frame, appendTexture, frame.totalAbsorb, totalAbsorb)
end

--WARNING: This function is very similar to the function UnitFrameUtil_UpdateFillBar in UnitFrame.lua.
--If you are making changes here, it is possible you may want to make changes there as well.
function RayCompactUnitFrameUtil_UpdateFillBar(frame, previousTexture, bar, amount, barOffsetXPercent)
...
function RayCompactUnitFrame_UpdateRoleIcon(frame)
...
function RayCompactUnitFrame_UpdateReadyCheck(frame)
...
function RayCompactUnitFrame_FinishReadyCheck(frame)
	if ( not frame.readyCheckIcon)  then
		return;
	end
	if ( frame:IsVisible() ) then
		frame.readyCheckDecay = RAY_CUF_READY_CHECK_DECAY_TIME;
...
function RayCompactUnitFrame_CheckReadyCheckDecay(frame, elapsed)
...
function RayCompactUnitFrame_UpdateCenterStatusIcon(frame)
...
function RayCompactUnitFrame_UpdateClassificationIndicator(frame)
	if (frame.classificationIndicator) then
		if (frame.optionTable.showPvPClassificationIndicator and RayCompactUnitFrame_UpdatePvPClassificationIndicator(frame)) then
			return;
...
function RayCompactUnitFrame_UpdateWidgetSet(frame)
	if not frame.WidgetContainer then
		return;
	end
...
function RayCompactUnitFrame_ClearWidgetSet(frame)
	if frame.WidgetContainer then
...
do
	local function RayCompactUnitFrame_ParseAllAuras(frame, displayOnlyDispellableDebuffs, ignoreBuffs, ignoreDebuffs, ignoreDispelDebuffs)
	...
	local function RayCompactUnitFrame_UpdateAurasInternal(frame, unitAuraUpdateInfo)
		...
		if unitAuraUpdateInfo == nil or unitAuraUpdateInfo.isFullUpdate or frame.debuffs == nil then
			RayCompactUnitFrame_ParseAllAuras(frame, displayOnlyDispellableDebuffs, ignoreBuffs, ignoreDebuffs, ignoreDispelDebuffs);
			debuffsChanged = true;
			buffsChanged = true;
			dispelsChanged = true;
		else
		...
		if debuffsChanged then
			...
			frame.debuffs:Iterate(function(auraInstanceID, aura)
				...
				if frame.optionTable.debuffBlacklist[aura.spellId] then
					return false;
				end
				local debuffFrame = frame.debuffFrames[frameNum];
				RayCompactUnitFrame_UtilSetDebuff(debuffFrame, aura);
				frameNum = frameNum + 1;

				-- if aura.isBossAura then
				-- 	-- Boss auras are about twice as big as normal debuffs, so we may need to display fewer buffs
				-- 	local bossDebuffScale = (debuffFrame.baseSize + RAY_BOSS_DEBUFF_SIZE_INCREASE)/debuffFrame.baseSize;
			...
			RayCompactUnitFrame_HideAllDebuffs(frame, frameNum);
		end
		...
		if buffsChanged then
			...
			frame.buffs:Iterate(function(auraInstanceID, aura)
				...
				if frame.optionTable.buffBlacklist[aura.spellId] then
					return false;
				end
				local buffFrame = frame.buffFrames[frameNum];
				RayCompactUnitFrame_UtilSetBuff(buffFrame, aura);
				frameNum = frameNum + 1;

				return false;
			end);

			RayCompactUnitFrame_HideAllBuffs(frame, frameNum);
		end
		...
		if dispelsChanged then
			...
			for _, auraTbl in pairs(frame.dispels) do
				...
				if auraTbl:Size() ~= 0 then
					local dispelDebuffFrame = frame.dispelDebuffFrames[frameNum];
					RayCompactUnitFrame_UtilSetDispelDebuff(dispelDebuffFrame, auraTbl:GetTop());
					frameNum = frameNum + 1;
				end
			end

			RayCompactUnitFrame_HideAllDispelDebuffs(frame, frameNum);
		end
	end

	function RayCompactUnitFrame_UpdateAuras(frame, unitAuraUpdateInfo)
		RayCompactUnitFrame_UpdateAurasInternal(frame, unitAuraUpdateInfo);
		RayCompactUnitFrame_UpdateClassificationIndicator(frame);
	end
end
...
function RayCompactUnitFrame_UpdatePvPClassificationIndicator(frame)
...
--Utility Functions
function RayCompactUnitFrame_HideAllBuffs(frame, startingIndex)
	if frame.buffFrames then
		for i=startingIndex or 1, #frame.buffFrames do
			frame.buffFrames[i].auraInstanceID = nil;
...
function RayCompactUnitFrame_HideAllDebuffs(frame, startingIndex)
	if frame.debuffFrames then
		for i=startingIndex or 1, #frame.debuffFrames do
			frame.debuffFrames[i].auraInstanceID = nil;
...
function RayCompactUnitFrame_HideAllDispelDebuffs(frame, startingIndex)
	if frame.dispelDebuffFrames then
		for i=startingIndex or 1, #frame.dispelDebuffFrames do
			frame.dispelDebuffFrames[i].auraInstanceID = nil;
...
function RayCompactUnitFrame_UtilSetBuff(buffFrame, aura)
...
function RayCompactUnitFrame_UtilSetDebuff(debuffFrame, aura)
	...
	-- if ( aura.isBossAura ) then
	-- 	local size = min(debuffFrame.baseSize + RAY_BOSS_DEBUFF_SIZE_INCREASE, debuffFrame.maxHeight);
...
function RayCompactUnitFrame_UtilSetDispelDebuff(dispelDebuffFrame, aura)
	dispelDebuffFrame:SetAlpha(1);
	--dispelDebuffFrame:Show();
	dispelDebuffFrame.icon:SetTexture("Interface\\RaidFrame\\Raid-Icon-Debuff"..aura.dispelName);
	local color = DebuffTypeColor[aura.dispelName] or DebuffTypeColor["none"];
	local max_color = math.max(color.r, color.g, color.b);
	dispelDebuffFrame.glow:SetVertexColor(color.r / max_color, color.g / max_color, color.b / max_color);
	--dispelDebuffFrame.glow:SetBlendMode("ADD");
	dispelDebuffFrame.auraInstanceID = aura.auraInstanceID;
end

--Dropdown
function RayCompactUnitFrameDropDown_Initialize(self)
...
DefaultRayCompactUnitFrameOptions = {
	useClassColors = true,
...
RAY_NATIVE_UNIT_FRAME_HEIGHT = 36;
RAY_NATIVE_UNIT_FRAME_WIDTH = 72;
DefaultRayCompactUnitFrameSetupOptions = {
	width = 72,
	height = 36,
	maxBuffs = 5,
	maxDebuffs = 5,
...
function DefaultRayCompactUnitFrameSetup(frame, setupOptions, frameOptions)
	local options = {};
	for k, v in pairs(DefaultRayCompactUnitFrameSetupOptions) do options[k] = v end
	if ( setupOptions ) then 
		for k, v in pairs(setupOptions) do options[k] = v end
	end
	...
	local frameWidth = options.width;
	local frameHeight = options.height;
	local displayBorder = true;

	local componentScale = min(frameHeight / RAY_NATIVE_UNIT_FRAME_HEIGHT, frameWidth / RAY_NATIVE_UNIT_FRAME_WIDTH);
	...
	frame.background:SetColorTexture(0.05, 0.05, 0.05, 0.65);
	...
	frame.healthBar:SetStatusBarTexture(Media:Fetch("statusbar", "Clean"));
	...
	if ( frame.powerBar ) then
		...
		frame.powerBar:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -1, 0);
	...
	frame.name:SetPoint("BOTTOM", frame.healthBar, "CENTER");
	...
	local buffSize = 16 * componentScale;

	RayCompactUnitFrame_SetMaxBuffs(frame, options.maxBuffs);
	RayCompactUnitFrame_SetMaxDebuffs(frame, options.maxDebuffs);
	RayCompactUnitFrame_SetMaxDispelDebuffs(frame, options.maxDispelDebuffs);

	local buffPos, buffRelativePoint, buffOffset = "BOTTOMRIGHT", "BOTTOMLEFT", RAY_CUF_AURA_BOTTOM_OFFSET;
	frame.buffFrames[1]:ClearAllPoints();
	frame.buffFrames[1]:SetPoint(buffPos, frame.healthBar, "BOTTOMRIGHT", -1, buffOffset);
	for i=1, #frame.buffFrames do
		if ( i > 1 ) then
			frame.buffFrames[i]:ClearAllPoints();
			frame.buffFrames[i]:SetPoint(buffPos, frame.buffFrames[i - 1], buffRelativePoint, -1, 0);
		end
		frame.buffFrames[i]:SetSize(buffSize, buffSize);
		if ( i > frame.maxBuffs ) then
			frame.buffFrames[i]:Hide();
		else
			frame.buffFrames[i]:Show();
		end
	end

	local debuffPos, debuffRelativePoint, debuffOffset = "BOTTOMLEFT", "BOTTOMRIGHT", RAY_CUF_AURA_BOTTOM_OFFSET;
	frame.debuffFrames[1]:ClearAllPoints();
	frame.debuffFrames[1]:SetPoint(debuffPos, frame.healthBar, "BOTTOMLEFT", 1, debuffOffset);
	for i=1, #frame.debuffFrames do
		if ( i > 1 ) then
			frame.debuffFrames[i]:ClearAllPoints();
			frame.debuffFrames[i]:SetPoint(debuffPos, frame.debuffFrames[i - 1], debuffRelativePoint, 1, 0);
		end
		frame.debuffFrames[i]:SetSize(buffSize, buffSize);
		if ( i > frame.maxDebuffs ) then
			frame.debuffFrames[i]:Hide();
		else
			frame.debuffFrames[i]:Show();
		end
	end
	...
	for i=1, #frame.dispelDebuffFrames do
		...
		frame.dispelDebuffFrames[i].glow:SetSize(frameWidth * 2, frameHeight * 3);
		frame.dispelDebuffFrames[i]:SetSize(18, 18);
		if ( i > frame.maxDispelDebuffs ) then
			frame.dispelDebuffFrames[i]:Hide();
		else
			frame.dispelDebuffFrames[i]:Show();
		end
	end
	...
	frame.mouseoverHighlight:SetColorTexture(1, 1, 1, 0.1);
	frame.mouseoverHighlight:SetAllPoints(frame);

	frame.castTargetHighlight:SetColorTexture(1, 1, 1, 0.1);
	frame.castTargetHighlight:SetAllPoints(frame);
	...
	if ( displayBorder ) then
		...
		frame.horizTopBorder:SetPoint("TOPLEFT", frame, "TOPLEFT", -borderOffset, borderOuter);
		frame.horizTopBorder:SetPoint("BOTTOMRIGHT", frame, "TOPRIGHT", borderOffset, borderOffset);
		frame.horizTopBorder:SetColorTexture(0, 0, 0);
		frame.horizTopBorder:Show();

		frame.horizBottomBorder:ClearAllPoints();
		frame.horizBottomBorder:SetPoint("TOPLEFT", frame, "BOTTOMLEFT", -borderOffset, -borderOffset);
		frame.horizBottomBorder:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", borderOffset, -borderOuter);
		frame.horizBottomBorder:SetColorTexture(0, 0, 0);
		frame.horizBottomBorder:Show();

		frame.vertLeftBorder:ClearAllPoints();
		frame.vertLeftBorder:SetPoint("TOPLEFT", frame, "TOPLEFT", -borderOuter, borderOuter);
		frame.vertLeftBorder:SetPoint("BOTTOMRIGHT", frame, "BOTTOMLEFT", -borderOffset, -borderOuter);
		frame.vertLeftBorder:SetColorTexture(0, 0, 0);
		frame.vertLeftBorder:Show();

		frame.vertRightBorder:ClearAllPoints();
		frame.vertRightBorder:SetPoint("TOPLEFT", frame, "TOPRIGHT", borderOffset, borderOuter);
		frame.vertRightBorder:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", borderOuter, -borderOuter);
		frame.vertRightBorder:SetColorTexture(0, 0, 0);
		frame.vertRightBorder:Show();

		frame.horizDivider:ClearAllPoints();
		frame.horizDivider:SetPoint("TOPLEFT", frame.healthBar, "BOTTOMLEFT", 0, 0);
		frame.horizDivider:SetPoint("TOPRIGHT", frame.healthBar, "BOTTOMRIGHT", 0, 0);
		frame.horizDivider:SetHeight(borderWidth);
		frame.horizDivider:SetColorTexture(0, 0, 0);
	...
	local options = {};
	for k, v in pairs(DefaultRayCompactUnitFrameOptions) do options[k] = v end
	if ( frameOptions ) then
		for k, v in pairs(frameOptions) do options[k] = v end
	end
	RayCompactUnitFrame_SetOptionTable(frame, options)
end

DefaultRayCompactMiniFrameOptions = {
	displaySelectionHighlight = true,
	displayAggroHighlight = true,
	displayName = true,
	fadeOutOfRange = true,
...
DefaultRayCompactMiniFrameSetUpOptions = {
	height = 18,
	width = 72,
	displayBorder = true,
}

function DefaultRayCompactMiniFrameSetup(frame, setupOptions)
	local options = setupOptions or DefaultRayCompactMiniFrameSetUpOptions;
	frame:SetAlpha(1);
	...
	RayCompactUnitFrame_SetOptionTable(frame, DefaultRayCompactMiniFrameOptions)
end

DefaultRayCompactNamePlateFriendlyFrameOptions = {
	useClassColors = true,
	displaySelectionHighlight = true,
...
DefaultRayCompactNamePlateEnemyFrameOptions = {
	displaySelectionHighlight = true,
	displayAggroHighlight = false,
...
DefaultRayCompactNamePlatePlayerFrameOptions = {
...
DefaultRayCompactNamePlateFrameSetUpOptions = {
	healthBarHeight = 4,
	healthBarAlpha = 0.75,
	castBarHeight = 8,
...
DefaultRayCompactNamePlatePlayerFrameSetUpOptions = {
	healthBarHeight = 4,
	healthBarAlpha = 1,
	castBarHeight = 8,
	castBarFontHeight = 10,
...
function DefaultRayCompactNamePlateFrameSetup(frame, options)
	...
	DefaultRayCompactNamePlateFrameSetupInternal(frame, DefaultRayCompactNamePlateFrameSetUpOptions, options);
	DefaultRayCompactNamePlateFrameAnchors(frame);
end

function DefaultRayCompactNamePlateFrameAnchors(frame)
	...
	DefaultRayCompactNamePlateFrameAnchorInternal(frame, DefaultRayCompactNamePlateFrameSetUpOptions);
end

function DefaultRayCompactNamePlateFriendlyFrameSetup(frame)
	DefaultRayCompactNamePlateFrameSetup(frame, DefaultRayCompactNamePlateFriendlyFrameOptions);
end

function DefaultRayCompactNamePlateEnemyFrameSetup(frame)
	DefaultRayCompactNamePlateFrameSetup(frame, DefaultRayCompactNamePlateEnemyFrameOptions);
end

function DefaultRayCompactNamePlatePlayerFrameAnchor(frame)
	PixelUtil.SetPoint(frame.healthBar, "LEFT", frame, "LEFT", 12, 5);
	PixelUtil.SetPoint(frame.healthBar, "RIGHT", frame, "RIGHT", -12, 5);

	DefaultRayCompactNamePlateFrameAnchorInternal(frame, DefaultRayCompactNamePlatePlayerFrameSetUpOptions);
end

function DefaultRayCompactNamePlatePlayerFrameSetup(frame)
	DefaultRayCompactNamePlateFrameSetupInternal(frame, DefaultRayCompactNamePlatePlayerFrameSetUpOptions, DefaultRayCompactNamePlatePlayerFrameOptions);
	DefaultRayCompactNamePlatePlayerFrameAnchor(frame);
end

function DefaultRayCompactNamePlateFrameSetupInternal(frame, setupOptions, frameOptions)
	...
	RayCompactUnitFrame_SetOptionTable(frame, frameOptions);
end

function DefaultRayCompactNamePlateFrameAnchorInternal(frame, setupOptions)
...
function RayCompactAura_OnLoad(frame)
	frame.tooltipOwned = false;
	frame.tooltipDeferred = false;
	frame.tooltipShown = false;

	frame.icon:SetTexCoord(0.05, 0.95, 0.05, 0.95);
	
	-- frame.dropDown = frame:GetParent().dropDown;
	-- RayCompactUnitFrame_SetUpClicks(frame, frame:GetParent());
end

function RayCompactAura_UpdateTooltip(frame, type, owned)
	local shouldOwn = owned;
	local shouldDefer = shouldOwn and frame.auraInstanceID == nil;
	local shouldShow = shouldOwn and not shouldDefer;

	-- Start by leaving the old regime
	if ( frame.tooltipDeferred ~= shouldDefer and not shouldDefer ) then
		frame:GetParent():GetScript("OnLeave")(frame:GetParent(), motion);
	end

	if ( shouldShow ~= frame.tooltipShown and not shouldShow ) then
		GameTooltip:Hide();
	end

	-- Then enter the new regime
	if ( frame.tooltipDeferred ~= shouldDefer and shouldDefer ) then
		frame:GetParent():GetScript("OnEnter")(frame:GetParent(), motion);
	end

	if ( shouldShow ~= frame.tooltipShown and shouldShow ) then
		GameTooltip:SetOwner(frame, "ANCHOR_RIGHT", 0, 0);
	end

	if ( shouldShow ) then
		if ( type == AuraUtil.AuraUpdateChangedType.Debuff ) then
			if ( frame.isBossBuff ) then
				GameTooltip:SetUnitBuffByAuraInstanceID(frame:GetParent().displayedUnit, frame.auraInstanceID, frame.filter);
			else
				GameTooltip:SetUnitDebuffByAuraInstanceID(frame:GetParent().displayedUnit, frame.auraInstanceID, frame.filter);
			end
		end
		if ( type == AuraUtil.AuraUpdateChangedType.Buff ) then
			GameTooltip:SetUnitBuffByAuraInstanceID(frame:GetParent().displayedUnit, frame.auraInstanceID, frame.filter);
		end
		if ( type == AuraUtil.AuraUpdateChangedType.DispelDebuff ) then
			GameTooltip:SetUnitDebuffByAuraInstanceID(frame:GetParent().displayedUnit, frame.auraInstanceID, "RAID");
		end
	end

	frame.tooltipOwned = shouldOwn;
	frame.tooltipDeferred = shouldDefer;
	frame.tooltipShown = shouldShow;
end

function RayCompactUnitFrame_UpdateMouseover(frame)
	if ( UnitIsUnit("mouseover", frame.unit) ) then
